...
'''
    ...
    This is the same, although adapted, version.py as seen in salt's source code.
...
def __get_version(version, version_info):
    ...
    try:
        ...
        if parsed_version_info != version_info:
            warnings.warn(
                'In order to get the proper salt-ci version with the '
                'git hash you need to update salt-ci\'s local git '
...
def versions_report():
    ...
    yield fmt.format('Salt-CI', __version__, pad=padding)
