...
class Organization(db.Model):
    ...
    def ghi(self):
        ...
        '''
        if not hasattr(self, '__ghi'):
            try:
                self.__ghi = g.identity.github.get_organization(self.login)
...
class Repository(db.Model):
    ...
    def __init__(self, id, name, full_name, url, description,
        ...
        if not hasattr(self, '__ghi'):
            try:
                if self.organization is not None:
                    self.__ghi = self.organization.ghi.get_repo(self.name)
                else:
                    self.__ghi = g.identity.github.get_user().get_repo(self.name)
            except GithubException, exc:
                log.warning('The user probably does not have the necessary github permissions')
                log.exception(exc)
                if exc.status == 404:
    ...
    def __hook_url(self, kind):
        return url_for(
            'hooks.{0}'.format(kind),
            login=self.owner and self.owner.login or g.identity.account.login,
            reponame=self.name,
